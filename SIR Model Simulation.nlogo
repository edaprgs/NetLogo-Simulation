globals [
  initial-susceptible
  transmission-rate ; effective contact rate
  recovery-rate
...
to setup
  ...
  set recovery-rate 0.0666666667 ; 1/15 recovery rate
  set reproduction-rate 6.0 ; ratio of effective contact rate (0.4) to the recovery rate (1/15)
  setup-turtles
...
to setup-turtles
  create-turtles initial-susceptible [
    set state "S"
    set shape "person"
    set size 1.3
    set color 65
  ...
  ask n-of 1 turtles [
    set state "I" ; infect one initial individual
    set color 15
  ]
end
...
to check-infection
  ...
  let probability-of-infection transmission-rate * infected-neighbors / count turtles
  if lockdown-effect [
    set probability-of-infection probability-of-infection / 2 ; Reduce infection rate during lockdown
  ]
...
381
461
Exponential Growth Rate of Disease
NIL
NIL
0.0
10.0
...
"recovered" 1.0 0 -987046 true "" "plot count turtles with [ state = \"R\" ]"
